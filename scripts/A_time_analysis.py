...
def mapper(line):
	...
	date = datetime.fromtimestamp(float(block_timestamp)).strftime('%d/%m/%Y')
	val = float(value)
	return (date, (val, 1))
...
# map to strings
result.map(lambda data: ','.join(str(d) for d in data))
#result.saveAsTextFile('/user/twm31/ml/out/A_number_of_transactions.csv')
